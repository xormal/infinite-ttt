...
"""Бесконечные крестики‑нолики (терминальная игра).

Логика игры вынесена в чистые функции, что позволяет покрывать её unit‑тестами
без необходимости запускать curses. Точка входа запускает пользовательский
...
# ---------------------------------------------------------------------------
EMPTY = ' '
PLAYER = 'X'      # Символ человека (или первый игрок в двух‑игроковом режиме)
COMPUTER = 'O'    # Символ компьютера (или второй игрок)

# Направления: горизонталь, вертикаль и две диагонали
DIRS: List[Tuple[int, int]] = [(1, 0), (0, 1), (1, 1), (1, -1)]
...
# ---------------------------------------------------------------------------
def check_triples(board: Board, symbol: str) -> List[List[Tuple[int, int]]]:
    """Возвращает список троек координат, образующих линию из *symbol*.

    Каждая тройка – список из трёх позиций ``(x, y)``. Перекрывающиеся
    тройки учитываются отдельно, как в оригинальной игре.
    """
    triples: List[List[Tuple[int, int]]] = []
    for (x, y), val in board.items():
        if val != symbol:
...
def remove_triples(board: Board, triples: List[List[Tuple[int, int]]]) -> None:
    """Удаляет все клетки, входящие в любые тройки из *triples*.
    Словарь ``board`` изменяется «на месте».
    """
    for line in triples:
        for p in line:
            board.pop(p, None)
...
# ---------------------------------------------------------------------------
def _winning_moves(board: Dict[Tuple[int, int], str], candidates: List[Tuple[int, int]]) -> List[Tuple[int, int]]:
    """Возвращает позиции‑кандидаты, которые дают компьютеру мгновенную тройку.
    """
    wins: List[Tuple[int, int]] = []
    for pos in candidates:
        trial = dict(board)
...
def _threat_moves(board: Dict[Tuple[int, int], str], candidates: List[Tuple[int, int]]) -> List[Tuple[int, int]]:
    """Возвращает позиции, где игрок может выиграть в следующий ход.

    Компьютер должен заблокировать такие клетки.
    """
    threats: List[Tuple[int, int]] = []
    for pos in candidates:
        trial = dict(board)
...
def _heuristic_score(board: Dict[Tuple[int, int], str], pos: Tuple[int, int]) -> int:
    """Простая эвристика: считает количество линий из двух символов ``COMPUTER``
    (включая новую позицию) без вражеских символов.

    Чем больше таких линий, тем выше приоритет хода.
    """
    score = 0
    trial = dict(board)
    trial[pos] = COMPUTER
...
def computer_move(board: Board, cursor: Tuple[int, int], view_radius: int = DIFFICULTY.value) -> Optional[Tuple[int, int]]:
    """Определяет следующий ход компьютера.

    Приоритеты выбора:
    1. Ход, завершающий тройку (выигрыш).
    2. Блокировка немедленного выигрыша игрока.
    3. Ход, создающий наибольшее количество потенциальных двойных линий.
    4. Случайная свободная клетка в окрестности курсора.
    """
    cx, cy = cursor
    candidates: List[Tuple[int, int]] = []
    for dx in range(-view_radius, view_radius + 1):
...
# ---------------------------------------------------------------------------
def _init_colors() -> None:
    """Инициализирует цветовые пары curses согласно настройкам ``src.config``.
    """
    curses.start_color()
    curses.use_default_colors()
    curses.init_pair(COLOR_PLAYER, curses.COLOR_RED, -1)
...
def draw(stdscr: Any, board: Dict[Tuple[int, int], str], cursor: Tuple[int, int], offset: Tuple[int, int], scores: Dict[str, int], move_history: List[Tuple[Tuple[int, int], str]]) -> None:
    """Отрисовывает игровое поле без мерцания.

    Пустые клетки отображаются точкой ``.``. Клетка под курсором выделяется
    обратным видео.
    """
    stdscr.erase()
    h, w = stdscr.getmaxyx()
